...
def linear_regeression_feature_performance(x_train, y_train, x_cv, y_cv, selected_features):
    ...
    :return: plt.show()
    """
    index_selected_features = len(selected_features) - 1
    ...
    for d in range(1, 5):
        ...
        axes[d - 1].set_title(f'Train vs Validation for {d} degree')
        ...
        axes[d - 1].vlines(index_selected_features, 0, max_value, color='r', linestyle='--', label='Selected Features')
        axes[d - 1].legend()
